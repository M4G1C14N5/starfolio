...
pandoc "$SCRIPT_DIR/public/Thomas_Tsangou_Resume.md" \
  ...
  --from markdown-blank_lines_after_headers \
  --pdf-engine=pdflatex \
  -V geometry:"top=0.4in,bottom=0.4in,left=0.5in,right=0.5in" \
  -V fontsize=8pt \
  -V pagestyle=empty \
  -V "header-includes=\\usepackage{setspace}\\setstretch{0.9}\\usepackage{enumitem}\\setlist[itemize]{noscript,noitemsep,topsep=2pt,parsep=0pt,partopsep=0pt,leftmargin=*}\\setlist[enumerate]{noitemsep,topsep=2pt,leftmargin=*}" \
